...
- 支持多级嵌套回复（最多 5 层），超深的回复自动挂到允许的最深祖先下；深层回复串默认折叠，点击展开；
...
- **可信度排序**：顶层评论按"作者信用分 × 发言权重 − 时间衰减"排序，回复串内保持时间正序；
- **刷屏降权**：10 分钟内发言超过 5 条的账号自动进入 1 小时降权状态，期间其评论排序权重 × 0.25 并淡化展示，不阻断发言。
...
- [ ] 多级回复（最多 5 层）可用，深层回复串默认折叠可展开
- [ ] 顶层评论按可信度排序，刷屏账号的评论自动沉底并淡化
- [ ] 新用户评论进入待审，高信用用户即时可见
